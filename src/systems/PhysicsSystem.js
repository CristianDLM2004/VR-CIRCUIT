...
export class PhysicsSystem {
  constructor(scene, camera, appState, stateSyncSystem, interactionSystem) {
    ...
    // Reposo
    this.restLinearSpeed = 0.045
    this.restAngularSpeed = 0.18
    this.restHoldMs = 70
    this.sleepAfterMs = 100

    // Solo dar spin si de verdad hubo lanzamiento fuerte
    this.spinFromThrow = 4.0
    this.spinThrowSpeedThreshold = 1.2

    // Contacto
    this.contactAngularDamping = 0.35
    this.contactAngularStop = 0.12
    this.contactLinearStop = 0.018

    // Tip/settle
    this.tipOverDuration = 0.22
    this.tipOverAngleThreshold = THREE.MathUtils.degToRad(10)
    this.settleDuration = 0.10
  ...
  ensureBodyForMesh(mesh, initialVel = null) {
    ...
    if (initialVel) {
      ...
      if (speed > this.spinThrowSpeedThreshold) {
        ...
        body.angVel.normalize().multiplyScalar(Math.min(8, speed * this.spinFromThrow))
      } else {
  ...
  stepMesh(mesh, body, dt) {
    ...
    if (hit) {
      ...
      if (mesh.position.y <= targetY + eps && body.vel.y <= 0) {
        ...
        if (legacySpeed < 0.12) body.sleepMs += dt * 1000
        else body.sleepMs = 0
      }
    }
